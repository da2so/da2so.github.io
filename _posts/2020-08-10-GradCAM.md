...
Obtaining the neuron importance weights <span style="color:DodgerBlue">$w^{c}_k=\frac{1}{z} \sum_i\frac{1}{z}\sum_i\sum_j\frac{\partial y^c}{\partial A^{k}_V}$</span>, where $V$ is $i, j$.
...
\\[Grad-CAM L^c=ReLu(\sum_k w^c_k A^k)\\]
