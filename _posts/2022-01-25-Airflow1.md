...
위 그림에서 DAG의 example과 각 column이 의미하는 내용을 적어놓았습니다. schedule부분에서 crontab형태로 나타내며 [해당 사이트](https://crontab.guru/)에서 의미를 파악하시면 될것같습니다. 다음과 같은 그리고 example_bash_operator를 클릭하고 'Graph'라는 menu선택을 하면 다음과 같이 해당 workflow의 DAG를 확인가능합니다. 해당 example들은 db initialize를 통해 생성된 것이며 <span style="color:DodgerBlue">airflow dags list</span>명령어를 통하여 webserver에서 보이는 example들을 terminal에서 볼 수 있습니다. 
